...
function Utils.Framework.getTopMiners()
	local sql = [[SELECT U.charinfo, M.user_id, M.exp, SUM(C.amount) as ores_mined
		FROM mining_simulator_users M
		INNER JOIN players U ON M.user_id = U.citizenid
		LEFT JOIN mining_simulator_ores_mined C ON M.user_id = C.user_id
		WHERE M.exp > 0
		GROUP BY U.charinfo, M.user_id, M.exp
		ORDER BY M.exp DESC
		LIMIT 10]];
	local result = Utils.Database.fetchAll(sql,{});
	for k,v in ipairs(result) do
