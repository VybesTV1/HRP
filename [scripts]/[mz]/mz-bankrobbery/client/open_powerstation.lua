            ...
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    if Config.NotifyType == 'qb' then
                        QBCore.Functions.Notify(Lang:t("error.fuses_already_blown"), "error", 3500)
                    elseif Config.NotifyType == "okok" then
                        exports['okokNotify']:Alert("FUSES BLOWN", Lang:t("error.fuses_already_blown"), 3500, "error")
                    end
                end
            else
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify(Lang:t("error.minium_police_required", {police = Config.MinimumThermitePolice}), "error", 3500)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("NOT ENOUGH COPS", Lang:t("error.minium_police_required", {police = Config.MinimumThermitePolice}), 3500, "error")
                end
            end
        end
...
RegisterNetEvent('mz-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitesuccess', function(_, cb)
    QBCore.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
            ...
            while time > 0 do
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify(Lang:t("general.thermite_detonating_in_seconds", {time = time}), "success", 3500)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("DETONATE", Lang:t("general.thermite_detonating_in_seconds", {time = time}), 3500, "success")
                end
                Wait(1000)
                time -= 1
            end
            ...
            if currentStation ~= 0 then
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify(Lang:t("success.fuses_are_blown"), "success", 3500)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("FUSES BLOWN", Lang:t("success.fuses_are_blown"), 3500, "success")
                end
                TriggerServerEvent("mz-bankrobbery:server:SetStationStatus", currentStation, true)
            elseif currentGate ~= 0 then
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify(Lang:t("success.door_has_opened"), "success", 3500)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("DOOR OPEN", Lang:t("success.door_has_opened"), 3500, "success")
                end
